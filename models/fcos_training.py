...
class Fcos_Loss(nn.Module):
    ...
    def forward(self, preds, gt_boxes, classes=3):
...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio = 0.1, warmup_lr_ratio = 0.1, no_aug_iter_ratio = 0.3, step_num = 10):
    def yolox_warm_cos_lr(lr, min_lr, total_iters, warmup_total_iters, warmup_lr_start, no_aug_iter, iters):
        ...
        if iters <= warmup_total_iters:
            lr = lr#(lr - warmup_lr_start) * iters / float(warmup_total_iters) + warmup_lr_start
...
if __name__ == "__main__":
    ...
    lr_limit_min = 3e-4
    lr_limit_max = 5e-4
    lr = min(max(8 / 16 * Init_lr, lr_limit_min), lr_limit_max)
    Min_lr_fit = min(max(8 / 16 * Min_lr, lr_limit_min * 1), lr_limit_max * 0.5)
    lr_function = get_lr_scheduler("cos", lr=Init_lr, min_lr=Min_lr_fit, total_iters=450)
    for epoch in range(451):
        eta = lr_function(epoch)
