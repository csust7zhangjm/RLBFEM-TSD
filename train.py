...
from models.fcos_training import Fcos_Loss, get_lr_scheduler, set_optimizer_lr
from utils.callbacks import EvalCallback, LossHistory
from utils.dataloader import FcosDataset, fcos_dataset_collate
from utils.utilss import get_classes, show_config
...
if __name__ == "__main__":
    ...
    #       在终端中输入    CUDA_VISIBLE_DEVICES=0,1 python train2.py
    #   DDP模式：
    #       设置            distributed = True
    #       在终端中输入    CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 train2.py
    #---------------------------------------------------------------------#
    distributed     = False
    #---------------------------------------------------------------------#
    ...
    #---------------------------------------------------------------------#
    classes_path    = 'model_data/GTSDB.txt'
    #---------------------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = '/root/code/paper_1/model_data/pretrain.pth'
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    val_annotation_path     = 'GTSDB_test.txt'
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
                ...
                Init_lr_fit     = min(max(batch_size / nbs * Init_lr, lr_limit_min), lr_limit_max)
                # Min_lr_fit      = min(max(batch_size / nbs * Min_lr, lr_limit_min * 1e-2), lr_limit_max * 1e-2)
                Min_lr_fit      = min(max(batch_size / nbs * Min_lr, lr_limit_min * 1), lr_limit_max * 0.5)
                #---------------------------------------#
                #   获得学习率下降的公式
                #---------------------------------------#
                lr_scheduler_func = get_lr_scheduler(lr_decay_type, Init_lr_fit, Min_lr_fit, UnFreeze_Epoch)
            ...
            best_map, cur_mAP = fit_one_epoch(model_train, model, fcos_loss, loss_history, eval_callback, optimizer, epoch,
                    epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank, best_map)

            with open(save_file, "a") as f:
                f.write("epoch:" + str(epoch) + "  " + 'lr:' + str(lr_scheduler_func(epoch)) + '  ' + 'mAP05:' + str(cur_mAP))
                f.write('\r\n')
