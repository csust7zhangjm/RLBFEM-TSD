...
def generate_xml(img_name, coords, img_size, out_root_path):
    ...
    # 将DOM对象doc写入文件
    f = open(os.path.join(out_root_path, "shift_img_" + img_name[:-4] + '.xml'), 'w')
...
class ImgAugemention():
    ...
    def process_img(self, imgs_path, xmls_path, img_save_path, xml_save_path, angle_list, imgs):
    ...
    def addGaussi(self, img_path, xml_path, img_save_path, xml_save_path, imgs):
        ...
        for im in imgs:
            ...
            # 写入图像
            cv2.imwrite(img_save_path + '/' + "Gau_img_" + im, Gau_img)
            xml = xml_path + '/' + im[:-4] + ".xml"
            tree = ET.parse(xml)
            tree.write(xml_save_path +"/"+ "Gau_img_" + im[:-4] + ".xml")

    # 调整亮度
    def changeLight(self, img_path, xml_path, img_save_path, xml_save_path, imgs):
        ...
        for im in imgs:
            ...
            cv2.imwrite(img_save_path + '/' + "light_img_" + im, light_img)
            xml = xml_path + '/' + im[:-4] + ".xml"
            tree = ET.parse(xml)
            tree.write(xml_save_path + '/' + "light_img_" + im[:-4] + ".xml")

    # 平移
    def shift_pic_bboxes(self, xml_path, img_path, img_save_path, save_path_xml, xmls):
        # img_save_path=img_save_path+'shift'
        # save_path_xml=save_path_xml+'shift'
        for xml in xmls:
            x = xml_path + '/' + xml
            coords = parse_xml(x)  # 读xml文件
            img = cv2.imread(img_path + '/' + xml[:-4] + ".jpg")
            ...
            for bbox in bboxes:
                ...
                i += 1
            cv2.imwrite(img_save_path + '/' + "shift_img_" + xml[:-4] + ".jpg", shift_img)
            file = xml[:-4] + ".jpg"
            auged_img = shift_img
    ...
    # 裁剪
    def crop_img_bboxes(self, xml_path, img_path, img_save_path, save_path_xml, imgs):
        '''
        裁剪后的图片要包含所有的框
        输入:
            img:图像array
        ...
        for im in imgs:
            ...
            for bbox in bboxes:
                ...
                i += 1
            cv2.imwrite(img_save_path + '/' + "crop_img_" + im, crop_img)
            auged_img = crop_img
            auged_bboxes = crop_bboxes

            generate_xml(im, auged_bboxes, list(auged_img.shape), save_path_xml)


if __name__ == '__main__':
    img_aug = ImgAugemention()
    # 路径修改为自己的
    imgs_path = './VOCdevkit3/VOC2007/JPEGImages'
    xmls_path = './VOCdevkit3/VOC2007/Annotations'
    save_xml = './new/new_xmls'
    save_img = './new/new_imgs'

    train_path = './VOCdevkit3/VOC2007/ImageSets/Main/test.txt'
    f = open(train_path, "r")
    imgs = [line.strip() + ".jpg" for line in f]
    xmls = [line.replace(".jpg", ".xml") for line in imgs]
    f.close()
    print("start rorate!!!")
    angle_list = [90, 180]
    img_aug.process_img(imgs_path, xmls_path, save_img, save_xml, angle_list, imgs)
    #
    print("start addGaussi!!!")
    img_aug.addGaussi(imgs_path, xmls_path, save_img, save_xml, imgs)
    print("start changeLight!!!")
    img_aug.changeLight(imgs_path, xmls_path, save_img, save_xml, imgs)
    print("start shift_pic_bboxes!!!")
    img_aug.shift_pic_bboxes(xmls_path, imgs_path, save_img, save_xml, xmls)

    # print("start crop_img_bboxes!!!")
    # img_aug.crop_img_bboxes(xmls_path, imgs_path, save_img, save_xml, imgs)